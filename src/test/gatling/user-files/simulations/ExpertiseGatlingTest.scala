 ...
 */
class ExpertiseGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Expertise entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all expertise")
            .get("/api/expertise")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new expertise")
            .post("/api/expertise")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "title":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_expertise_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created expertise")
                .get("${new_expertise_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created expertise")
            .delete("${new_expertise_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
