<template>
  <div>
    <h2 id="page-heading" data-cy="JobHeading">
      <span v-text="$t('demoJhVueOAuthElasticSearch973App.job.home.title')" id="job-heading">Jobs</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('demoJhVueOAuthElasticSearch973App.job.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'JobCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-job">
            ...
            <span v-text="$t('demoJhVueOAuthElasticSearch973App.job.home.createLabel')"> Create a new Job </span>
          </button>
        </router-link>
      </div>
    </h2>
        ...
        <form name="searchForm" class="form-inline" v-on:submit.prevent="search(currentSearch)">
          <div class="input-group w-100 mt-3">
            <input
              ...
              name="currentSearch"
              id="currentSearch"
              v-bind:placeholder="$t('demoJhVueOAuthElasticSearch973App.job.home.search')"
    ...
    <div class="alert alert-warning" v-if="!isFetching && jobs && jobs.length === 0">
      <span v-text="$t('demoJhVueOAuthElasticSearch973App.job.home.notFound')">No jobs found</span>
    </div>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="demoJhVueOAuthElasticSearch973App.job.delete.question"
      ...
      <div class="modal-body">
        <p id="jhi-delete-job-heading" v-text="$t('demoJhVueOAuthElasticSearch973App.job.delete.question', { id: removeId })">
