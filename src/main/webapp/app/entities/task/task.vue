<template>
  <div>
    <h2 id="page-heading" data-cy="TaskHeading">
      <span v-text="$t('demoJhVueOAuthElasticSearch973App.task.home.title')" id="task-heading">Tasks</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('demoJhVueOAuthElasticSearch973App.task.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'TaskCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-task">
            ...
            <span v-text="$t('demoJhVueOAuthElasticSearch973App.task.home.createLabel')"> Create a new Task </span>
          </button>
        </router-link>
      </div>
    </h2>
        ...
        <form name="searchForm" class="form-inline" v-on:submit.prevent="search(currentSearch)">
          <div class="input-group w-100 mt-3">
            <input
              ...
              name="currentSearch"
              id="currentSearch"
              v-bind:placeholder="$t('demoJhVueOAuthElasticSearch973App.task.home.search')"
    ...
    <div class="alert alert-warning" v-if="!isFetching && tasks && tasks.length === 0">
      <span v-text="$t('demoJhVueOAuthElasticSearch973App.task.home.notFound')">No tasks found</span>
    </div>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="demoJhVueOAuthElasticSearch973App.task.delete.question"
      ...
      <div class="modal-body">
        <p id="jhi-delete-task-heading" v-text="$t('demoJhVueOAuthElasticSearch973App.task.delete.question', { id: removeId })">
