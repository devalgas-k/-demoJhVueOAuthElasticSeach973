<template>
  <div>
    <h2 id="page-heading" data-cy="SubjectHeading">
      <span v-text="$t('demoJhVueOAuthElasticSearch973App.subject.home.title')" id="subject-heading">Subjects</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('demoJhVueOAuthElasticSearch973App.subject.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'SubjectCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('demoJhVueOAuthElasticSearch973App.subject.home.createLabel')"> Create a new Subject </span>
          </button>
        </router-link>
      </div>
    </h2>
        ...
        <form name="searchForm" class="form-inline" v-on:submit.prevent="search(currentSearch)">
          <div class="input-group w-100 mt-3">
            <input
              ...
              name="currentSearch"
              id="currentSearch"
              v-bind:placeholder="$t('demoJhVueOAuthElasticSearch973App.subject.home.search')"
    ...
    <div class="alert alert-warning" v-if="!isFetching && subjects && subjects.length === 0">
      <span v-text="$t('demoJhVueOAuthElasticSearch973App.subject.home.notFound')">No subjects found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('demoJhVueOAuthElasticSearch973App.subject.name')">Name</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="demoJhVueOAuthElasticSearch973App.subject.delete.question"
          data-cy="subjectDeleteDialogHeading"
          v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-subject-heading" v-text="$t('demoJhVueOAuthElasticSearch973App.subject.delete.question', { id: removeId })">
